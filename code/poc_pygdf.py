...
logging.basicConfig(filename='/home/ubuntu/pygdf/logs/poc_pygdf.log', level=logging.INFO)
...
def launch_count():
    ...
    logging.info('##################')
    logging.info('Number of items in Data Frame is %s', len(df_ratings[df_ratings.columns[0]]))
...
def inner_join_ratings_users():
    joined = df_ratings.join(df_users, how='inner', lsuffix='_left', rsuffix='_right', sort=True)
    # print(joined[:10])


@timing
def outer_join_ratings_users():
    joined = df_ratings.join(df_users, how='outer', lsuffix='_left', rsuffix='_right', sort=True)
    # print(joined[:10])
